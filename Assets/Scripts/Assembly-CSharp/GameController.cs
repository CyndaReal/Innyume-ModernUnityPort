...
public class GameController : MonoBehaviour
{
	...
	private void FixedUpdate()
	{
		...
		{
			GameObject gameObject = Object.Instantiate(innyumePrefab, new Vector3(-145f, 20f, -80f), Quaternion.identity) as GameObject;
			gameObject.name = "innyume1";
			isAdd_Ga = true;
		}
		if (getItem[0] && !isAdd_Ri)
		{
			GameObject gameObject2 = Object.Instantiate(innyumePrefab, new Vector3(105f, 20f, -80f), Quaternion.identity) as GameObject;
		...
		if (getItem[0] && getItem[1] && getItem[2] && getItem[3] && getItem[4])
		{
			...
			for (int i = 0; i < 5; i++)
			{
				GameObject gameObject3 = GameObject.Find("Item_" + (i + 1) + "(Clone)");
				gameObject3.GetComponent<Renderer>().material = colorMAT;
				Object.Destroy(gameObject3.GetComponent<Light>());
			}
