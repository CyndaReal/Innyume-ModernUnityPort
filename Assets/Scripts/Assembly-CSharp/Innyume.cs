...
public class Innyume : MonoBehaviour
{
	...
	private void Start()
	{
		...
		beforePos = 1;
		NearTarget();
		GameObject gameObject = Object.Instantiate(eyePrefab, base.gameObject.transform.position, Quaternion.identity) as GameObject;
	...
	private void FixedUpdate()
	{
		...
		Vector3 position2 = AIPos[aimPos].transform.position;
		if (frame == 9)
		{
		...
		base.gameObject.transform.eulerAngles = new Vector3(0f, 0f - num + 90f, 0f);
	...
	private void NearTarget()
	{
		...
		for (int i = 1; i < 77; i++)
		{
			array[i] = Vector3.Distance(base.gameObject.transform.position, AIPos[i].transform.position);
		}
		float num = 1000f;
		int num2 = 1;
		for (int j = 1; j < 77; j++)
